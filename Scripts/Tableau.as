	...
	public class Tableau extends MovieClip {
		...
		public function deplacerJoueur(perso:Perso, dirX:Number, dirY:Number):void {
			...
			if(_jeu.getEstEnDialogue() == false){
				...
				if(posValide) {
					...
					if(dirX<0)direction = "Gauche";
					else if(dirX>0)direction = "Droite";
					else if(dirY<0)direction = "Haut";
					else if(dirY>0)direction = "Bas";
		...
		******************************************************************************/
		public function arreterJoueur(perso:Perso):void {
			if(perso.currentLabel.indexOf("Idle")==-1)perso.gotoAndStop("Idle"+perso.currentLabel.charAt(0)); //Cette ligne cherche si il y a le mot Idle dans le label. 
																	//Dans le cas ou il n'y est pas, on envoie le perso au Idle correspondant à son orientation actuelele
		}
